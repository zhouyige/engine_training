...
namespace Lean3D
{
	...
	uint32 OGLTextureManager::calcTextureSize(TextureFormats::List format, int width, int height, int depth)
	{
		...
		case TextureFormats::DXT1:
			return maxi(width / 4, 1) * maxi(height / 4, 1) * depth * 8;
		case TextureFormats::DXT3:
			return maxi(width / 4, 1) * maxi(height / 4, 1) * depth * 16;
											...
											, bool compress, bool sRGB)
	{
		...
		///??????????????????????????????不打算加  不应该启用15个以上纹理单元
		//if (_actTexlist.texHandle[15] != 0)
		//{
		//	TextureBuffer &tex = _texturesReflist.getRef(_actTexlist.texHandle[15]);
		//	glBindTexture(tex.type, tex.texRef);
		//}
	...
	void OGLTextureManager::uploadTextureData(uint32 texHandle, int slice, int mipLevel, const void *pixelsData)
	{
		...
		int width = maxi(tex.width >> mipLevel, 1), height = maxi(tex.height >> mipLevel, 1);
		...
		{
			int depth = maxi(tex.depth >> mipLevel, 1);
			if (compressed)
	...
	{
		uploadTextureData(texHandle, slice, mipLevel, pixelsData);
	}
	...
	uint32 OGLTextureManager::getOGLTexRef(uint32 handle)
	{
		TextureBuffer &tex = _texturesReflist.getRef(handle);
		return tex.texRef;
	}
